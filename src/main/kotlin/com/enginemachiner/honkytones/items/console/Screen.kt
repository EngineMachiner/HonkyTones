...
class DigitalConsoleScreenHandler(
    ...
    val stack = player.handItems.find { it.item is DigitalConsole }!!
    ...
    override fun onClosed(player: PlayerEntity) {
        ...
        super.onClosed(player);    val stack = inventory.getStack(0)
    ...
    companion object : ModID {
        ...
        val type = ScreenHandlerType( ::DigitalConsoleScreenHandler, FeatureFlags.VANILLA_FEATURES )
...
class DigitalConsoleScreen(
    ...
    override fun drawBackground( context: DrawContext, delta: Float, mouseX: Int, mouseY: Int ) {
        ...
        context.drawTexture( genericTexture, centerX + 140, centerY + 85, 7, 17, 18, 18 )

        context.drawTexture( genericTexture, centerX + 140, centerY + 79, 4, 0, 18, 6 )
        context.drawTexture( genericTexture, centerX + 140, centerY + 103, 4, 216, 18, 6 )

        context.drawTexture( genericTexture, centerX + 134, centerY + 79, 0, 0, 6, 24 )
        context.drawTexture( genericTexture, centerX + 158, centerY + 79, 170, 0, 6, 24 )

        context.drawTexture( genericTexture, centerX + 134, centerY + 103, 0, 216, 6, 18 )
        context.drawTexture( genericTexture, centerX + 158, centerY + 103, 170, 216, 6, 18 )

        centerX = ( width - 256 ) / 2;        centerY = ( height - 256 ) / 2

        context.drawTexture( consoleBackTexture, centerX, centerY, 0, 0, 256, 256 )

        centerX = ( width - 64 ) / 2;                   centerY = ( height - 64 ) / 2

        // C
        renderKey( firstKeyTexture, keyBindings[0], context, centerX - 83, centerY + 23, 64, 64 )

        val flatsCenterX = ( width - 32 ) / 2;          val flatsCenterY = ( height - 32 ) / 2

        // D_, D, E_, E, F
        renderKey( flatKeyTexture, keyBindings[1], context, flatsCenterX - 68, flatsCenterY + 7, 32, 32 )
        renderKey( middleKeyTexture, keyBindings[2], context, centerX - 55, centerY + 23, 64, 64 )
        renderKey( flatKeyTexture, keyBindings[3], context, flatsCenterX - 41, flatsCenterY + 7, 32, 32 )
        renderKey( lastKeyTexture, keyBindings[4], context, centerX - 27, centerY + 23, 64, 64 )
        renderKey( lastKeyFlipTexture, keyBindings[5], context, centerX + 1, centerY + 23, 64, 64 )

        // G_, A_, B_
        for ( i in 0..2 ) renderKey( flatKeyTexture, keyBindings[ 6 + i * 2 ], context, flatsCenterX + 15 + 28 * i, flatsCenterY + 7, 32, 32 )

        // G, A, B
        renderKey( middleKeyTexture, keyBindings[7], context, centerX + 29, centerY + 23, 64, 64 )
        renderKey( middleKeyTexture, keyBindings[9], context, centerX + 57, centerY + 23, 64, 64 )
        renderKey( firstKeyFlipTexture, keyBindings[11], context, centerX + 85, centerY + 23, 64, 64 )
    ...
    override fun render( context: DrawContext, mouseX: Int, mouseY: Int, delta: Float ) {
        ...
        context.drawText( textRenderer, octaveTitle, ( width * 0.5f + 10 ).toInt(), ( height * 0.5f - 65 ).toInt(), Color.WHITE.rgb, false )

        drawTime(context);       recordCheckbox!!.renderButton( context, mouseX, mouseY, delta )
    ...
    private fun drawTime(context: DrawContext) {
        ...
        context.drawText( textRenderer, format, ( width * 0.5f + 3 ).toInt(), ( height * 0.5f - 49 ).toInt(), Color.WHITE.rgb, false )
...
class PickStackScreenHandler( syncID: Int, playerInventory: PlayerInventory ) : ScreenHandler( type, syncID ) {
    ...
    private var consoleStack = player.handItems.find { it.item is DigitalConsole }!!
    ...
    companion object : ModID {
        ...
        val type = ScreenHandlerType( ::PickStackScreenHandler, FeatureFlags.VANILLA_FEATURES )
        ...
        fun networking() {
            ...
            ClientPlayNetworking.registerGlobalReceiver(id) {
                ...
                client.send {
                    ...
                    val console = player()!!.handItems.find { it.item is DigitalConsole }!!
...
class PickStackScreen(
    ...
    override fun drawBackground( context: DrawContext, delta: Float, mouseX: Int, mouseY: Int ) {
        ...
        context.drawTexture( genericTexture, centerX, centerY + 20, 0, 126, w, h )
        context.drawTexture( genericTexture, centerX, centerY + 18, 0, 0, w, 3 )
