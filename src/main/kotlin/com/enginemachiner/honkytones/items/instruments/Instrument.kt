...
open class Instrument( private val damage: Float, val speed: Float, material: ToolMaterial )
    ...
    : TypedActionResult<ItemStack>? {
        ...
        if ( !tag.getBoolean("isOnUse") ) {

            tag.putBoolean("isOnUse", true)
            if ( world!!.isClient ) {
                ...
                b = !tag.getBoolean("isOnKeyBindUse")
                b = b && tag.getString("Action") != "Ranged"
    ...
    override fun onStoppedUsing( stack: ItemStack?, world: World?, user: LivingEntity?,
        ...
        if ( tag.getBoolean("isOnUseKeyBind") ) return

        tag.putBoolean("isOnUse", false)
            ...
            if ( instStack.isNotEmpty() ) {
                ...
                for ( stack in instStack ) {
                    ...
                    if ( menuKeyBind.wasPressed() ) {
                        if (instStack.size < size) client.openScreen( InstrumentsScreen(stack) )
                        else player.sendMessage(instrumentsMessage, true)
                    }

                    if ( replayKeyBind.wasPressed() ) {
                        val seq = tag.getString("Sequence")
                        tag.putString("TempSequence", seq)
                        Network.sendNbtToServer(tag)
                    }

                    val b = tag.getBoolean("isOnUseKeyBind")
                    val inst = stack.item as Instrument

                    if ( playKeyBind.isPressed && !b ) {

                        tag.putBoolean("isOnUseKeyBind", true)
                    ...
                    if ( !playKeyBind.isPressed && b ) {
                        ...
                        tag.putBoolean("isOnUseKeyBind", false)
                        tag.putBoolean("isOnUse", false)
                    }
    ...
    private fun rangedAttack( stack: ItemStack, user: PlayerEntity ) {
        ...
        val tag = stack.tag!!.getCompound(Base.MOD_NAME)
        if (tag.getString("Action") != "Ranged") return
    ...
    private fun loadSequence(stack: ItemStack): Boolean {
        ...
        val tag = stack.tag!!.getCompound(Base.MOD_NAME)
        var subsequence = tag.getString("TempSequence")
        ...
        tag.putString("TempSequence", subsequence)
        return true
...
class Trombone : Instrument( 5f, -3f, MusicalRedstone() ) {
    ...
    override fun use(world: World?, user: PlayerEntity, hand: Hand?): TypedActionResult<ItemStack>? {
        ...
        val stack = user.getStackInHand(hand)
        val tag = stack.tag!!.getCompound(Base.MOD_NAME)
        val isOnUse = tag.getBoolean("isOnUse")
